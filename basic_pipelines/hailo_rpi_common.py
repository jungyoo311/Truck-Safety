...
def display_user_data_frame(user_data: app_callback_class):
    while user_data.running:
        frame = user_data.get_frame()
        if frame is not None:
            # Define the center and axes for the 270° 15m range arc
            height, width = frame.shape[:2]
            center = (width // 2, height)
            axes = (int(width * 0.5), int(height * 0.5))  # Adjust as necessary for visibility

            # Draw the intercepted arc on the frame
            cv2.ellipse(
                frame,
                center,
                axes,
            ...
            # Show the frame with the intercepted arc overlay
            cv2.imshow("Frame with ROI", frame)
            cv2.waitKey(1)  # Short delay to update the display
    cv2.destroyAllWindows()
