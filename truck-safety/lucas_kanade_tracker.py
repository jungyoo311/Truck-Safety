...
class LucasKanadeTracker:
    ...
    def track(self, frame):
            ...
            current_time = time.time()      #modified
            dt = current_time - self.prev_time if self.prev_time else 1.0  #modified
            distances = np.linalg.norm(new_pts - self.prev_pts, axis=2).flatten()  #modified
            avg_movement = np.mean(distances) if len(distances) > 0 else 0.0       #modified

            #modified - compute speed in px/sec
            speed = avg_movement / dt  #modified

            #modified - compute acceleration
            if self.prev_speed is not None:  #modified
                acceleration = (speed - self.prev_speed) / dt  #modified
            else:  #modified
                acceleration = 0.0           #modified

            #modified - update times and speeds
            self.prev_time = current_time    #modified
            self.prev_speed = speed          #modified
