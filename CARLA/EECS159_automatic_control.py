...
yolo_model = YOLO("E:\\SeniorDesignProject\\models\\yolov10s.pt")
...
class World(object):
    ...
    def __init__(self, carla_world, hud, args):
        ...
        # ---------------------------- camera end ECU data init --------------------------
        self.log_file_path = 'E:/SeniorDesignProject/CARLA/output/speed_accel_log.json'
        self.log_interval = 0.1     # Log every 0.1 seconds
        self.last_log_time = 0
        self.log_data = {
            "metadata": {
                "simulation_time_start": 0,  # Will be set when simulation starts
                "logging_interval": self.log_interval,
                "map_name": None,  # Will be set after initializing map
                "vehicle_type": None,  # Will be set when the player is spawned
                "sensor_config": {
                    "camera_resolution": (hud.dim[0], hud.dim[1]),
                    "camera_fps": 30
                }
            },
            "data": []  # Stores logged speed and acceleration data AND YOLO detections now
        }
        # --------------------------------------------------------------------------------
    ...
    def tick(self, clock):
        ...
        if self.log_data["metadata"]["simulation_time_start"] == 0:
            self.log_data["metadata"]["simulation_time_start"] = self.hud.simulation_time
            if self.map:
                self.log_data["metadata"]["map_name"] = self.map.name
            if self.player:
                self.log_data["metadata"]["vehicle_type"] = get_actor_display_name(self.player)
                
         # Record speed and acceleration
        current_time = self.hud.simulation_time
        if current_time - self.last_log_time >= self.log_interval:
            velocity = self.player.get_velocity()
            acceleration = self.player.get_acceleration()
            speed = 3.6 * math.sqrt(velocity.x**2 + velocity.y**2 + velocity.z**2)  # km/h
            accel = math.sqrt(acceleration.x**2 + acceleration.y**2 + acceleration.z**2)  # m/s²
            
            # YOLO detections from the camera manager
            yolo_detections = []
            if self.camera_manager.recording and self.camera_manager.detections:
                yolo_detections = self.camera_manager.detections[-1]["detections"]  # Get latest detections
    ...
    def destroy(self):
        """Destroys all actors"""
        # Write speed and acceleration data to file
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYUP:
                if self._is_quit_shortcut(event.key):
                    ...
                    if hasattr(self.world.camera_manager, 'toggle_recording'):
                        self.world.camera_manager.toggle_recording()
                elif event.key == pygame.K_c:  # Switch camera type
                    self.world.camera_manager.next_sensor()
                elif event.key == pygame.K_v:  # Change camera perspective
...
class CameraManager(object):
    ...
    def __init__(self, parent_actor, hud):
        ...
        #------------------ own (forgot what used for) ------------
        self.frame_count = 0
        #---------------- recording -------------------------------
        self.video_writer = None
        self.video_path = 'E:/SeniorDesignProject/CARLA/output/camera_video.avi'
        
         # YOLO Detections Log
        self.detection_log_path = 'E:/SeniorDesignProject/CARLA/output/yolo_detections.json'
    ...
    def toggle_recording(self):
        ...
        if self.recording:
            ...
            fourcc = cv2.VideoWriter_fourcc(*'XVID')  # Use codec
            self.video_writer = cv2.VideoWriter(
                self.video_path,
                fourcc,
                30.0,  # FPS (adjust as necessary)
                (self.hud.dim[0], self.hud.dim[1])  # Video resolution
            )
            self.hud.notification('Recording Started')
        else:
            # Release the writer when recording stops
            if self.video_writer:
                self.video_writer.release()
            self.video_writer = None
            self.hud.notification('Recording Stopped')
    ...
    def _parse_image(weak_self, image):
        ...
        if not self:
            return
            # Convert CARLA image to a NumPy array      # NOTE: There was probably code here that I removed. I forgot what this comment was for
        ...
        """ Below has been commented. The code below is for saving LIDAR data and single frames to a directory """
        # if self.recording:
        #     if self.sensors[self.index][0].startswith('sensor.lidar'):
        #         # Save point cloud data to a file
        #         np.savetxt(f'E:/SeniorDesignProject/CARLA/output/lidar_data/lidar_{image.frame}.txt', points, fmt='%.3f')
